...
if [[ -z $1 ]]; then
    echo "queue parameter is required (e.g., scc)"
    exit 1
fi
...
for dt in "${DATASETS[@]}"
    ...
    # SGE: qsub -cwd -V -q queue -N jobname -pe smp threads -l h_vmem=XXXM -l h_rt=runtime -o logfile -j y command
    qsub -cwd -V -q "$1" -N "${arrDataset[0]}" -pe smp $JOB_CPU -l h_vmem=${JOB_MEMORY}M -l h_rt=${JOB_RUNTIME} -o ${arrDataset[0]}.log -j y ./biobtree $BB_DEFAULT_PARAM -d ${arrDataset[1]} --out-dir "${2}/${arrDataset[0]}" -idx ${arrDataset[0]} update
    done
...
for ens in "${ENS_DATASETS[@]}"
    ...
    rm -rf ${2}/${ens}
    mkdir -p ${2}/${ens}
    qsub -cwd -V -q "$1" -N "$ens" -pe smp $JOB_CPU -l h_vmem=${JOB_MEMORY}M -l h_rt=${JOB_RUNTIME} -o ${ens}.log -j y ./biobtree $BB_DEFAULT_PARAM $BB_ENSEMBL_PARAM -d $ens --out-dir "${2}/${ens}" -idx $ens update
    waitJob $ens
    sleep 300
    done
