...
python -m kg_export nodes  --index-dir $IDX \
...
python -m kg_export edges    --index-dir $IDX --id-map out/kg/id_map.tsv --out out/kg/edges_direct.tsv  --stats out/kg/edges.stats.json
python -m kg_export reified  --index-dir $IDX --id-map out/kg/id_map.tsv --out out/kg/edges_reified.tsv --stats out/kg/reified.stats.json
python -m kg_export go       --index-dir $IDX --id-map out/kg/id_map.tsv --nodes-out out/kg/go_nodes.tsv --edges-out out/kg/go_edges.tsv
# 3. assemble
python -m kg_export assemble \
...
| `__main__.py` | CLI: `python -m kg_export {nodes,edges,reified,go,assemble} ...`. |
...
```bash
python3 -m unittest discover -s src/scripts/kg_export/tests -p "test_*.py"
```
