...
echo "### 2/7 GO $(date +%T)"
$PY -m kg_export go --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export refseq --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export ontology --index-dir $IDX \
...
if [ "$WITH_PREDICTIONS" = "1" ]; then
  ...
  $PY -m kg_export reified --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export attributes --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export structure --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export nodeattrs --index-dir $IDX --id-map $O/id_map.tsv.gz \
...
$PY -m kg_export subgraph \
  --nodes $O/dump/nodes.tsv.gz --edges $O/dump/edges.tsv.gz --config src/scripts/kg_export/mappings/subgraph.yaml --workers 8 \
...
$PY -m kg_export showcase --index-dir $IDX --id-map $O/id_map.tsv.gz \
  --config src/scripts/kg_export/mappings/showcase.yaml \
