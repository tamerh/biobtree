...
clinical_trials, cellxgene_celltype, ctd_gene_interaction, civic, ortholog/paralog.
    ...
    python3 -m unittest kg_export.tests.test_coverage_tier_a -v
"""
...
from kg_export.edges import build_edges
from kg_export.mesh import build_mesh
from kg_export.predicates import PredicateMap
from kg_export.reified import build_reified_edges
from kg_export.structure import build_structure

REPO_ROOT = Path(__file__).resolve().parents[4]
CONF = REPO_ROOT / "conf"
CATS = Path(__file__).resolve().parents[1] / "mappings" / "categories.yaml"
PREDS = Path(__file__).resolve().parents[1] / "mappings" / "predicates.yaml"
...
class TierACoverageTests(unittest.TestCase):
    ...
    def test_hpa_pathology_deferred(self):
        ...
        skip = (yaml.safe_load((Path(__file__).resolve().parents[1] / "mappings" / "coverage_skip.yaml").read_text()) or {}).get("skip", {})
    ...
    def test_node_attributes_from_property_json(self):
        ...
        from kg_export.attributes import build_attributes, load_attributes
        from kg_export import kgx
