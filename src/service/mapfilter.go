...
func (s *Service) execCelGo(query *query.Query, targetXref *pbuf.Xref) (bool, error) {
	...
	**/
	...
	case "cellosaurus":
		if attr := targetXref.GetCellosaurus(); attr != nil {
			attr.Id = id
			evalMap["cellosaurus"] = attr
		}
	case "bindingdb":
	...
	case "faers":
		if attr := targetXref.GetFaers(); attr != nil {
			attr.Id = id
			evalMap["faers"] = attr
		}
	case "faers_reaction":
		if attr := targetXref.GetFaersReaction(); attr != nil {
			attr.Id = id
			evalMap["faers_reaction"] = attr
		}
	case "biogrid":
	...
	case "ncrna_drug":
		if attr := targetXref.GetNcrnaDrug(); attr != nil {
			attr.Id = id
			evalMap["ncrna_drug"] = attr
		}
	case "alliance_disease":
		if attr := targetXref.GetAllianceDisease(); attr != nil {
			attr.Id = id
