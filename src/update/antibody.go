...
func (a *antibody) mapIndicationToOntology(antibodyID string, indication string, ontologyDatasetID uint32, fr string, ontologyName string) {
	// Shared normalization cascade (medical_mappings.go) - same resolver used by
	// clinical_trials, intogen and civic.
	found := collectOntologyIDs(a.d, a.medicalTermMappings, indication, ontologyDatasetID)
	if len(found) > 0 {
		a.createOntologyXrefs(antibodyID, fr, found, ontologyName)
	}
}
