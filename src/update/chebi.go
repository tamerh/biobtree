...
func (c *chebi) update() {
	...
	chebiPath := dataconf[c.source]["path"]
	chebiFiles := strings.Split(dataconf[c.source]["files"], ",")
	...
	for _, name := range chebiFiles {
		br, _, ftpFile, localFile, fr, _ := c.d.getDataReaderNew(c.source, c.d.ebiFtp, c.d.ebiFtpPath, chebiPath+name)
