#!/usr/bin/env python3
"""
ChEMBL Document Reference Data Extraction Script

Fetches full ChEMBL document data from ChEMBL REST API for each document ID.
Creates reference_data.json with complete document information for test validation.

ChEMBL API: https://www.ebi.ac.uk/chembl/api/data/document/<CHEMBL_ID>
"""
...
class ChEMBLDocumentExtractor:
    """Extracts complete ChEMBL document data from official ChEMBL REST API"""
    ...
    def fetch_entry(self, chembl_id: str) -> Optional[Dict]:
        """
        Fetch COMPLETE ChEMBL document data from REST API
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # (API may have it nested or in different format)
                data["id"] = chembl_id
    ...
    def extract_all(self, id_file: Path, output_file: Path):
        """Extract all document entries and save to JSON"""

        # Read IDs
        with open(id_file, 'r') as f:
            ids = [line.strip() for line in f if line.strip()]

        print(f"Extracting {len(ids)} ChEMBL document entries from API...")
...
def main():
    ...
    id_file = script_dir / "chembl_document_ids.txt"
    ...
    if not id_file.exists():
        ...
        print("  2. ./biobtree -d 'chembl_document' test")
        print("  3. cp test_out/reference/chembl_document_ids.txt tests/chembl_document/")
        return 1

    extractor = ChEMBLDocumentExtractor()
