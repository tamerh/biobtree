#!/usr/bin/env python3
"""
Extract reference data from DIAMOND Similarity test database build.
Reads DIAMOND protein IDs from test_out/reference/diamond_similarity_ids.txt
"""
...
def main():
    ...
    ids_file = project_root / "test_out" / "reference" / "diamond_similarity_ids.txt"
    output_file = script_dir / "reference_data.json"

    if not ids_file.exists():
        print(f"Error: {ids_file} not found")
        print("Build the test database first: ./biobtree -d diamond_similarity test")
        return 1
    ...
    if not protein_ids:
        print("Error: No protein IDs found in diamond_similarity_ids.txt")
        return 1
