#!/usr/bin/env python3
"""
Extract full HGNC data for reference IDs

This fetches complete HGNC entries for the IDs in hgnc_ids.txt
...
class HGNCExtractor:
    """Extract HGNC reference data from REST API"""

    BASE_URL = "https://rest.genenames.org/fetch/hgnc_id"
    ...
    def fetch_entry(self, hgnc_id: str) -> Optional[Dict]:
        """Fetch single HGNC entry from API"""
        url = f"{self.BASE_URL}/{hgnc_id}"
        ...
        try:
            ...
            print(f"  ✗ Error fetching {hgnc_id}: {e}", file=sys.stderr)
            self.stats["failed"] += 1
            return None
    ...
    def extract_all(self) -> List[Dict]:
        """Extract all entries"""
        print("═" * 60)
        print("  HGNC Reference Data Extraction")
        print("═" * 60)
        print()

        # Load IDs
        ids = self.load_ids()
        print(f"Found {len(ids)} HGNC IDs to fetch")
        print()

        # Fetch each entry
        print("Fetching data from HGNC REST API...")
        print()

        entries = []
        for i, hgnc_id in enumerate(ids, 1):
            print(f"[{i:3d}/{len(ids)}] Fetching {hgnc_id} ... ", end="", flush=True)

            entry = self.fetch_entry(hgnc_id)
    ...
    def print_summary(self):
        ...
        if self.stats["success"] > 0:
            ...
            with open(self.output_file) as f:
                ...
                if data["entries"]:
                    ...
                    print(f"  HGNC ID:    {first.get('hgnc_id')}")
                    print(f"  Symbol:     {first.get('symbol')}")
                    print(f"  Name:       {first.get('name')}")
                    print(f"  Locus type: {first.get('locus_type')}")
        ...
        print()
        print("Files created:")
        print("  - hgnc_ids.txt         (100 HGNC IDs)")
        print("  - reference_data.json  (Full HGNC entries)")
        print()
...
def main():
    ...
    ids_file = script_dir / "hgnc_ids.txt"
    output_file = script_dir / "reference_data.json"

    try:
        extractor = HGNCExtractor(ids_file, output_file)
