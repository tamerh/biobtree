...
Tests OBA (Ontology of Biological Attributes) dataset processing using the common test framework.
...
class OBATests:
    """OBA custom tests (in addition to declarative tests)"""

    def __init__(self, runner: TestRunner):
        self.runner = runner

    @test
    def test_biological_attribute_with_name(self):
        """Check OBA term has biological attribute name"""
        ...
        if not entry:
            return False, "No OBA term with descriptive name in reference"

        oba_id = entry["id"]
        term_name = entry.get("name", "unknown")

        data = self.runner.lookup(oba_id)

        if not data or not data.get("results"):
            return False, f"No results for {oba_id}"

        return True, f"{oba_id} has name: {term_name[:60]}..."

    @test
    def test_term_with_synonyms(self):
        """Check OBA term has synonyms"""
        ...
        if not entry:
            return False, "No OBA term with synonyms in reference"
        ...
        return True, f"{oba_id} has {synonym_count} synonym(s) (e.g., '{first_synonym}')"
...
def main():
    ...
    # Add custom tests
    custom_tests = OBATests(runner)
    for test_method in [custom_tests.test_biological_attribute_with_name,
