...
Tests OBI (Ontology for Biomedical Investigations) dataset processing using the common test framework.
...
class OBITests:
    """OBI custom tests (in addition to declarative tests)"""

    def __init__(self, runner: TestRunner):
        self.runner = runner

    @test
    def test_investigation_term_with_name(self):
        """Check OBI term has investigation name"""
        ...
        if not entry:
            return False, "No OBI term with descriptive name in reference"

        obi_id = entry["id"]
        term_name = entry.get("name", "unknown")

        data = self.runner.lookup(obi_id)

        if not data or not data.get("results"):
            return False, f"No results for {obi_id}"

        return True, f"{obi_id} has name: {term_name[:60]}..."

    @test
    def test_term_with_synonyms(self):
        """Check OBI term has synonyms"""
        ...
        return True, f"{obi_id} has {synonym_count} synonym(s) (e.g., '{first_synonym}')"
...
def main():
    ...
    # Add custom tests
    custom_tests = OBITests(runner)
    for test_method in [custom_tests.test_investigation_term_with_name,
