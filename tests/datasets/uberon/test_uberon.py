...
Tests UBERON (Uber-anatomy Ontology) dataset processing using the common test framework.
...
class UberonTests:
    """UBERON custom tests (in addition to declarative tests)"""

    def __init__(self, runner: TestRunner):
        self.runner = runner

    @test
    def test_anatomy_term_with_name(self):
        """Check UBERON term has descriptive anatomical name"""
        # Find a term with a descriptive name (not just generic)
        entry = next(
        ...
        if not entry:
            return False, "No UBERON term with descriptive name in reference"

        uberon_id = entry["id"]
        term_name = entry.get("name", "unknown")

        data = self.runner.lookup(uberon_id)

        if not data or not data.get("results"):
            return False, f"No results for {uberon_id}"

        return True, f"{uberon_id} has name: {term_name[:60]}..."

    @test
    def test_term_with_synonyms(self):
        """Check UBERON term has synonyms"""
        ...
        if not entry:
            return False, "No UBERON term with synonyms in reference"
        ...
        return True, f"{uberon_id} has {synonym_count} synonym(s) (e.g., '{first_synonym}')"

    @test
    def test_text_search_by_name(self):
        """Test text search by anatomy term name"""
        ...
        if not entry:
            return False, "No multi-word anatomy term found in reference"
        ...
        # Verify the correct UBERON ID is in results
        found = any(
            result.get("identifier") == uberon_id
            for result in data.get("results", [])
        )

        if found:
            return True, f"Text search for '{term_name[:50]}...' found {uberon_id}"
        else:
            return False, f"Text search for '{term_name}' didn't return expected ID {uberon_id}"

    @test
    def test_text_search_by_synonym(self):
        """Test text search by anatomy term synonym"""
        # Find a term with synonyms by first checking what's actually in the database
        # (Reference data may have more complete synonym info than the OWL file)
        entry = next(
            (e for e in self.runner.reference_data
             if e.get("synonyms") and len(e.get("synonyms", [])) > 0),
            None
        ...
        id_data = self.runner.lookup(uberon_id)
        if not id_data or not id_data.get("results"):
            return False, f"Could not lookup {uberon_id} to verify synonyms"
        ...
        if not db_synonyms:
            ...
            entry = next(
                (e for e in self.runner.reference_data
                 if e["id"] != uberon_id and e.get("synonyms") and len(e.get("synonyms", [])) > 0),
                None
            )
            if entry:
                uberon_id = entry["id"]
        ...
        if found:
            return True, f"Synonym search for '{synonym[:50]}...' found {uberon_id}"
        else:
            return False, f"Synonym search for '{synonym}' didn't return expected ID {uberon_id}"
...
def main():
    ...
    # Add custom tests
    custom_tests = UberonTests(runner)
    for test_method in [custom_tests.test_anatomy_term_with_name,
