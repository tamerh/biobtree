#!/usr/bin/env python3
"""
Extract full UniRef100 data for reference IDs

This fetches complete UniRef100 entries for the IDs in uniref100_ids.txt
...
class UniRef100Extractor:
    """Extract UniRef100 reference data from REST API"""
    ...
    def fetch_entry(self, uniref_id: str) -> Optional[Dict]:
        """Fetch single UniRef100 entry from API"""
    ...
    def extract_all(self) -> List[Dict]:
        """Extract all entries"""
        print("═" * 60)
        print("  UniRef100 Reference Data Extraction")
        print("═" * 60)
        print()

        # Load IDs
        ids = self.load_ids()
        print(f"Found {len(ids)} UniRef100 IDs to fetch")
        print()
    ...
    def print_summary(self):
        ...
        print()
        print("Files created:")
        print("  - uniref100_ids.txt      (100 UniRef100 IDs)")
        print("  - reference_data.json  (Full UniRef100 entries)")
        print()
...
def main():
    ...
    ids_file = script_dir / "uniref100_ids.txt"
    output_file = script_dir / "reference_data.json"

    try:
        extractor = UniRef100Extractor(ids_file, output_file)
