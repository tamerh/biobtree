#!/usr/bin/env python3
"""
Extract full UniRef50 data for reference IDs

This fetches complete UniRef50 entries for the IDs in uniref50_ids.txt
...
class UniRef50Extractor:
    """Extract UniRef50 reference data from REST API"""
    ...
    def fetch_entry(self, uniref_id: str) -> Optional[Dict]:
        """Fetch single UniRef50 entry from API"""
    ...
    def extract_all(self) -> List[Dict]:
        """Extract all entries"""
        print("═" * 60)
        print("  UniRef50 Reference Data Extraction")
        print("═" * 60)
        print()

        # Load IDs
        ids = self.load_ids()
        print(f"Found {len(ids)} UniRef50 IDs to fetch")
        print()
    ...
    def print_summary(self):
        ...
        print()
        print("Files created:")
        print("  - uniref50_ids.txt      (100 UniRef50 IDs)")
        print("  - reference_data.json  (Full UniRef50 entries)")
        print()
...
def main():
    ...
    ids_file = script_dir / "uniref50_ids.txt"
    output_file = script_dir / "reference_data.json"

    try:
        extractor = UniRef50Extractor(ids_file, output_file)
