...
Tests XCO (Experimental Conditions Ontology) dataset processing using the common test framework.
...
class XCOTests:
    """XCO custom tests (in addition to declarative tests)"""

    def __init__(self, runner: TestRunner):
        self.runner = runner

    @test
    def test_experimental_condition_with_name(self):
        """Check XCO term has experimental condition name"""
        ...
        if not entry:
            return False, "No XCO term with descriptive name in reference"

        xco_id = entry["id"]
        term_name = entry.get("name", "unknown")

        data = self.runner.lookup(xco_id)

        if not data or not data.get("results"):
            return False, f"No results for {xco_id}"

        return True, f"{xco_id} has name: {term_name[:60]}..."

    @test
    def test_term_with_synonyms(self):
        """Check XCO term has synonyms"""
        ...
        if not entry:
            return False, "No XCO term with synonyms in reference"
        ...
        return True, f"{xco_id} has {synonym_count} synonym(s) (e.g., '{first_synonym}')"
...
def main():
    ...
    # Add custom tests
    custom_tests = XCOTests(runner)
    for test_method in [custom_tests.test_experimental_condition_with_name,
