...
$PY -m tools.kg_export go --index-dir $IDX --id-map $O/id_map.tsv \
...
$PY -m tools.kg_export edges --index-dir $IDX --id-map $O/id_map.tsv --datasets $EDGE_DS \
  --out $O/edges_direct.tsv --stats $O/edges.stats.json

echo "### 4/5 reified edges $(date +%T)"
$PY -m tools.kg_export reified --index-dir $IDX --id-map $O/id_map.tsv --datasets $REIFIED_DS \
