...
echo "### 1/6 nodes (peak-mem) $(date +%T)"
...
echo "### 3/6 RefSeq (transcript/protein/ncRNA nodes + edges) $(date +%T)"
...
echo "### 6/6 assemble (stub-nodes + gzip) $(date +%T)"
$PY -m tools.kg_export assemble \
  --nodes $O/nodes_core.tsv.gz,$O/go_nodes.tsv.gz,$O/refseq_nodes.tsv.gz \
  --edges $O/edges_direct.tsv.gz,$O/edges_reified.tsv.gz,$O/go_edges.tsv.gz,$O/refseq_edges.tsv.gz \
